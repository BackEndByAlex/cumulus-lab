...
This is the manual, hand-typed precursor to `01-single-server/terraform` and `01-single-server/ansible`: the same SSH-accessible Ubuntu server, built one `openstack` command at a time instead of through code. It's still a valid reference if I want to understand what's happening under the hood before Terraform and Ansible automate it.

See [troubleshooting.md](troubleshooting.md) for problems encountered while running through these commands.

## Command Reference

The full, working command sequence, in order. Run these in Git Bash on Windows (or any bash shell on Linux/macOS).
...
# IMPORTANT (Windows only) — fix line endings immediately, see troubleshooting.md below
sed -i 's/\r$//' mykey.pem
