# Terraform (01-single-server) — Command Reference

See [structure.md](structure.md) for a walkthrough of what each `.tf` file actually does. This page covers the workflow commands used to build and manage the stack.

The Terraform workflow commands used to build and manage this stack, in order. Run these in Git Bash on Windows (or any bash shell on Linux/macOS), from inside `01-single-server/terraform/`, after sourcing the Cumulus `openrc` file (see [manual-cli/openstack-cli.md](../manual-cli/openstack-cli.md) for that step, it still applies, since the OpenStack provider reads its credentials from the same `OS_*` environment variables).
