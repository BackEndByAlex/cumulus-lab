# Terraform (01-single-server) — Troubleshooting

Real problems I ran into building this Terraform stack, with causes and fixes. Search this page for your exact error message.
...
**Symptom:** after copying the project into the WSL Ubuntu filesystem (see `docs/general/wsl-setup.md`) and running `terraform init` fresh there, it failed with an HCL syntax error pointing at `volume.tf`.
...
**Cause:** this happened in a brand-new terminal session where I hadn't sourced the OpenStack credentials yet. Since moving the project into WSL, the credentials file lives outside the project at `~/.cumulus-secrets/project-openrc.sh` (see `docs/general/secrets-and-security.md`), which makes it easy to forget the sourcing step entirely, it's no longer sitting right there in the project directory as a visual reminder. The `OS_*` environment variables it sets only exist in the exact shell session where I sourced it, so any new terminal starts out with none of them set (same root cause as troubleshooting entry 1 above, just the specific error message this now produces post-WSL-move).
