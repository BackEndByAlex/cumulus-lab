# Terraform (01-single-server) — File Structure

This recreates the same infrastructure as the manual CLI setup (see [manual-cli/openstack-cli.md](../manual-cli/openstack-cli.md)), network, router, security group, server, and floating IP, but entirely through Terraform `.tf` files in `01-single-server/terraform/`. Instead of typing `openstack` commands one at a time, the desired end state is described in HCL and Terraform figures out how to get there with `terraform plan` and `terraform apply`.

The `.tf` files themselves don't contain comments. All the explanations live in this doc instead.

A walkthrough of every `.tf` file, explaining what each resource block does and why it's built the way it is. Terraform doesn't care what order files load in, or that "main.tf" is named that. It reads every `.tf` file in the directory and combines them into one configuration. The files are split up by topic purely to keep things readable for a human.
...
- `variable "web_ports_open"` — the list of ports the server's security group should allow inbound traffic on.
...
The student's own private network and subnet. This replaces the `mynetwork` / `mysubnet` that were previously created by hand via `openstack network create` and `openstack subnet create` (see [manual-cli/openstack-cli.md](../manual-cli/openstack-cli.md)). Terraform now owns creating and destroying these.
...
- `openstack_networking_secgroup_rule_v2.web_rules` — one ingress rule per port in `var.web_ports_open`, generated with `count`.
  - `count` tells Terraform "create this many copies of this resource". Here, one copy per entry in the ports list.
  - Inside the block, `count.index` is the position of the current copy in that list, so `var.web_ports_open[count.index]` picks out each port in turn. Without `count`, you'd have to copy-paste this rule once per port by hand.
...
- `openstack_compute_instance_v2.campus_web_server` — the actual server.
  - `image_name` — exact image name as listed by `openstack image list` on Cumulus. `image_name` is simpler to read and write than `image_id`, and there's no ambiguity risk here since Cumulus doesn't have multiple images sharing this exact name.
  - `flavor_name = "c1-r1-d10"` — 1 vCPU / 1024MB RAM / 10GB disk.
  - `key_pair = "mykey"` — an existing keypair already created by hand via `openstack keypair create` (see [manual-cli/openstack-cli.md](../manual-cli/openstack-cli.md)), with its matching `mykey.pem` private key the student already has. Terraform deliberately does NOT create a keypair resource here. Doing so would either fail (name already exists) or generate a brand-new keypair whose private key doesn't match the one already saved.
...
- `openstack_networking_floatingip_associate_v2.campus_server_floating_ip_associate` — attaches the floating IP to the port the server is plugged into.
  ...
  - `depends_on = [openstack_networking_router_interface_v2.campus_router_interface]` — forces Terraform to wait until the router interface has fully attached the private subnet before attempting the association. See [troubleshooting.md](troubleshooting.md) for the actual error this prevents.
...
Optional block storage, currently commented out. This is from the "maybe also" list in the workshop PDF, not a required part of the stack. It would give the server a separate disk on top of the 10GB that already comes from the flavor's own disk size.
...
- `openstack_blockstorage_volume_v3.campus_web_server_volume` — creates the volume itself. A volume is just a virtual disk that lives on its own, separate from the server. It exists on its own timeline: I can create it, destroy the server, and the volume stays around with its data intact, as long as I don't destroy the volume too.
  ...
  - `availability_zone = "nova"` — the Cinder (block storage) availability zone. This is a separate namespace from Nova (compute) availability zones, so it doesn't have to match the server's `"Education"` zone. Cumulus only exposes one Cinder AZ, and it's called `"nova"`. See [troubleshooting.md](troubleshooting.md) entry 4 for how I found this out.
...
- [commands.md](commands.md) — the Terraform workflow commands (init, plan, apply, destroy, etc.)
- [troubleshooting.md](troubleshooting.md) — real problems encountered building this stack
